"""Refresh the project landing page after all results have been generated."""
import json
...
def main():
    ...
    p = [json.loads((root / f'wrap-{seed}/parity.json').read_text()) for seed in (42,43)]
    success = [r['target_on_baseline_correct']['accuracy'] for v in p for r in v['parity_augmented_steering']]
    low, high = min(success)*100, max(success)*100
    Path('README.md').write_text(f'''# How a Transformer Learns a Map
